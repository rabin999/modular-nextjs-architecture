...
async function createFeature() {
    ...
    const featureId = parentPath ? `${parentPath.replace(/\//g, '-')}-${name}` : name
...
async function updateRegistryLazy(name: string, parentPath: string, manifestName: string, type: 'features' | 'capabilities') {
    ...
    const importPath = parentPath ? `./${parentPath}/${name}/manifest` : `./${name}/manifest`

    const registryKey = parentPath ? `${parentPath.replace(/\//g, '-')}-${name}` : name
